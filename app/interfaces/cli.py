...
def analyze(
    ...
    export: Annotated[Optional[ExportFormat], typer.Option("--export", help="Auto-export after analysis.")] = None,
    no_charts: Annotated[bool, typer.Option("--no-charts", help="Skip chart generation.")] = False,
    news_days: Annotated[Optional[int], typer.Option("--news-days", help="Override NEWS_DAYS_BACK.")] = None,
    price_days: Annotated[Optional[int], typer.Option("--price-days", help="Override PRICE_WINDOW_DAYS.")] = None,
) -> None:
...
def compare(
    ...
    base: Annotated[Optional[str], typer.Option("--base", help="Base date (YYYY-MM-DD).")] = None,
    compare_date: Annotated[Optional[str], typer.Option("--compare", help="Compare date (YYYY-MM-DD).")] = None,
    fmt: Annotated[OutputFormat, typer.Option("--format", "-f", help="Output format.")] = OutputFormat.md,
    language: Annotated[str, typer.Option("--language", "-l", help="Report language.")] = config.DEFAULT_LANGUAGE,
    no_narrative: Annotated[bool, typer.Option("--no-narrative", help="Skip LLM narrative.")] = False,
    no_auto_generate: Annotated[bool, typer.Option("--no-auto-generate", help="Don't auto-generate missing report.")] = False,
    out: Annotated[Path, typer.Option("--out", "-o", help="Reports directory.")] = Path("reports"),
) -> None:
...
@cli_app.command()
def export(
    report: Annotated[Optional[str], typer.Option("--report", help="Report stem (e.g. 2026-04-06_BTC).")] = None,
    symbol: Annotated[Optional[str], typer.Option("--symbol", "-s", help="Crypto ticker.")] = None,
    report_date: Annotated[Optional[str], typer.Option("--date", help="Report date (YYYY-MM-DD).")] = None,
    source: Annotated[Optional[OutputFormat], typer.Option("--source", help="Source format to export from (md or json).")] = None,
    ...
    resolved_symbol, resolved_date = _run(
        _resolve_export_target, report, symbol, report_date
    )
...
def charts(
    ...
    window_days: Annotated[Optional[int], typer.Option("--window-days", help="Chart display window.")] = None,
    out: Annotated[Path, typer.Option("--out", "-o", help="Output directory.")] = Path("reports"),
) -> None:
